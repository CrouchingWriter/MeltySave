...
**[Download v0.8.0](https://github.com/CrouchingWriter/MeltySave/releases/tag/v0.8.0)** · **[Installation](INSTALL.txt)** · **[User guide](USER-GUIDE.md)** · **[Report a bug](https://github.com/CrouchingWriter/MeltySave/issues)**

Download `MeltySave-v0.8.0.zip` from the release assets. GitHub's automatically generated **Source code** archives do not contain the installable plugin.

> Prerelease: the latest physical HMD interaction report is still awaiting confirmation on this build. See the validation boundary below.
...
Close the game, install/initialize BepInEx, then extract `MeltySave-v0.8.0.zip` into the folder containing `MeltyNight VR.exe`. The plugin should be at `BepInEx/plugins/MeltySave.dll`, beside `NVorbis.dll` and `NLayer.dll`. Back up existing plugin files and `MeltySave/saves` before updating.
